...
void DoubleSphereCamera::initialize_param(const std::vector<std::vector<cv::Point2d>>& pixels, const std::vector<cv::Point3d>& worlds)
{
    ...
    for(int i = 0; i < pixels_center.size(); i++)
    {
        ...
        // r13^2 + (BB-CC)*r13 - AA = 0
        std::vector<double> r13_sqrt, r13, r23;
        double tmp = std::sqrt(std::pow(BB-CC, 2) + 4*AA);
        if(CC - BB - tmp >= 0)
        {
            r13_sqrt.push_back(std::sqrt((CC-BB-tmp)/2));
            r13_sqrt.push_back(-std::sqrt((CC-BB-tmp)/2));
        }
        r13_sqrt.push_back(std::sqrt((CC-BB+tmp)/2));
        r13_sqrt.push_back(-std::sqrt((CC-BB+tmp)/2));
        ...
        for(int j = 0; j < Rt.size(); j++)
        {
            ...
            for(int k = 0; k < point_num; k++)
            {
                ...
                double rpho = u*u+v*v;
                double A_ = r12*worlds[k].x + r22*worlds[k].y + t2;
                double B_ = v * (r13_*worlds[k].x + r23_*worlds[k].y);
                double C_ = r11*worlds[k].x + r21*worlds[k].y + t1;
    ...
    fx_ /= pixels_center.size();
    fy_ /= pixels_center.size();
}
